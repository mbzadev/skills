...
Les skills classés dans `engineering/` et `productivity/` possèdent également une page dans `docs/<catégorie>/<nom-du-skill>.md`. L’arborescence de `docs/` reflète celle de `catalog/`. L’URL publiée reste `https://aihero.dev/skills-<nom-du-skill>`, quelle que soit la catégorie. Lorsqu’un skill promu est ajouté, renommé ou modifié, créez ou resynchronisez sa page selon [`.agents/writing-docs.md`](./.agents/writing-docs.md). Une page complète contient, dans cet ordre, les sections **Ce qu’il fait**, **Quand l’utiliser**, **Questions fréquentes** et **Indicateurs de réussite**. Les catégories non promues n’ont pas de page de documentation publique.
