...
description: "Câblez le croiseur de dépendances dans un dépôt TypeScript afin que chaque package soit un module profond - implémentation cachée dans des sous-dossiers, accessible uniquement via ses fichiers de point d'entrée. Invoqué par l'utilisateur."
---
...
**Des points d'entrée, pas un baril.** Parce que la surface publique est *chaque* fichier racine, un package peut exposer plusieurs petits points d'entrée (`index.ts`, `client.ts`, `server.ts`) au lieu de tout canaliser via un géant `index.ts`. Les fichiers Barrel qui réexportent un sous-arbre entier sont déconseillés : gardez les points d'entrée petits et masquez l'implémentation dans des sous-dossiers.
...
### 2. Installer le croiseur de dépendances
...
- `tests/example.test.ts` — importe **uniquement** `../index`  (un point d'entrée) et s'affirme contre la fonction publique.
...
3. Annuler l'importation approfondie. Exécutez à nouveau – cela doit **réussi**.
...
Écrivez un `README.md` **dans le dossier packages** (`<packages-root>/README.md`) — à côté des packages qu'il régit — couvrant : la `src/packages/<name>/` layout (points d'entrée à la racine, `lib/` pour l'implémentation, `tests/` pour les tests), "importer uniquement via les points d'entrée d'un package (sa racine fichiers)", et comment exécuter `lint:boundaries`. **Découragez explicitement les fichiers barils** : exposez plusieurs petits points d'entrée au lieu de réexporter un sous-arbre entier via un seul index. Conservez-le dans l'extrait de code copié ainsi que les quatre règles dans un paragraphe chacune.

Ajoutez ensuite un **pointeur de contexte** dans le fichier `AGENTS.md` du dépôt. Créez ce fichier s’il n’existe pas encore. Une seule ligne suffit, par exemple : `Les packages sont des modules profonds — consultez [src/packages/README.md](./src/packages/README.md) avant d’en ajouter ou d’en importer un.` L’agent peut ainsi découvrir la règle de délimitation avant de la violer.

**Terminé lorsque :** `<packages-root>/README.md` existe, déconseille les fichiers tonneaux et que le fichier `AGENTS.md` du dépôt y renvoie.

## Remarques

- Les références arrière `$1`  de la configuration (correspondance de groupe du croiseur de dépendances) sont ce qui permet à un paquet d'atteindre ses propres éléments internes alors que les étrangers ne le peuvent pas - ne les aplatissez pas dans des règles distinctes par paquet.
